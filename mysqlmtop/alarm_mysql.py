    ...
    if result <> 0:
        for line in result:
            ...
            if connect <> "success":
                    ...
                    param=(application_id,server_id,create_time,'mysql','connect',connect,'error','数据库服务器连接失败',send_mail, send_message)    
                    func.mysql_exec(sql,param)
            else:
                if int(alarm_connections)==1:
                    if int(connections)>=int(threshold_connections):
                        sql="insert into alarm(application_id,server_id,create_time,db_type,alarm_type,alarm_value,level,message,send_mail,send_message) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                        param=(application_id,server_id,create_time,'mysql','connections',connections,'warning','数据库总连接数过多',send_mail, send_message)                 
                        func.mysql_exec(sql,param)
                if int(alarm_active)==1:
                    if int(active)>=int(threshold_active):
                        sql="insert into alarm(application_id,server_id,create_time,db_type,alarm_type,alarm_value,level,message,send_mail,send_message) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                        param=(application_id,server_id,create_time,'mysql','active',active,'warning','数据库活动连接过多',send_mail, send_message)
                        func.mysql_exec(sql,param)
    else:
       pass
...
def get_alarm_mysql_replcation():
    sql="select a.application_id,a.server_id,a.create_time,a.slave_io_run,a.slave_sql_run,a.delay,b.send_mail,b.alarm_repl_status,b.alarm_repl_delay,b.threshold_repl_delay,b.send_message from mysql_replication a,servers b  where a.server_id=b.id and a.is_slave='1';"
    result=func.mysql_query(sql)
    if result <> 0:
        for line in result:
            application=line[0]
                ...
                if (slave_io_run== "Yes") and (slave_sql_run== "Yes"):
                    if alarm_repl_delay=="yes":
                        if int(delay)>=int(threshold_repl_delay):
                            sql="insert into alarm(application_id,server_id,create_time,db_type,alarm_type,alarm_value,level,message,send_mail,send_message) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                            param=(application,server_id,create_time,'mysql','delay',delay,'warning','数据库备库延时',send_mail,send_message)
                            func.mysql_exec(sql,param)
            else:
                sql="insert into alarm(application_id,server_id,create_time,db_type,alarm_type,alarm_value,level,message,send_mail,send_message) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
                param=(application,server_id,create_time,'mysql','replication','IO Thread:'+slave_io_run+',SQL Thread:'+slave_sql_run,'error','数据库同步进程停止',send_mail,send_message)
                func.mysql_exec(sql,param)
    else:
       pass
    ...
    if result <> 0:
        ...
        for line in result:
            ...
            if send_message_status==1:
                func.mysql_exec("insert into alarm_history(application_id,server_id,create_time,db_type,alarm_type,alarm_value,level,message,send_message,send_message_status) select application_id,server_id,create_time,db_type,alarm_type,alarm_value,level,message,send_message,1 from alarm;",'')
            elif send_message_status==0:
                func.mysql_exec("insert into alarm_history(application_id,server_id,create_time,db_type,alarm_type,alarm_value,level,message,send_message,send_message_status) select application_id,server_id,create_time,db_type,alarm_type,alarm_value,level,message,send_message,0 from alarm;",'')
            func.mysql_exec("delete from alarm",'')    
